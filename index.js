...
client.connect((err) => {
  ...
  !err ? console.log("database connected") : console.log("databasae connection failed");
  //get all product
  ...
  app.get("/buyProduct/:id", (req, res) => {
    productCollection.find({ _id: ObjectId(req.params.id) }).toArray((err, products) => {
      res.send(products);
    });
  });
      ...
      admin
        ...
        .then((decodedToken) => {
          ...
          if (tokenEmail == req.query.email) {
            orderDetails.find({ email: queryEmail }).toArray((err, orders) => {
              res.send(orders);
            });
          } else {
  ...
  app.post("/confirmOrder", (req, res) => {
    ...
    orderDetails.insertOne(newProduct).then((result) => {
  ...
  app.delete("/deleteItem/:id", (req, res) => {
    ...
    console.log(id);
    productCollection.deleteOne({ _id: id }).then((document) => res.send(document.value));
  });
});
